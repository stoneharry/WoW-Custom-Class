...
function newModelInt(parent,alpha,light,wSquish,hSquish,camera,scene)	-- creates a new model object that gets used internally but also can be altered after loading
	...
	mod:SetSize(scene:GetWidth() / wSquish, scene:GetHeight() / hSquish)
	mod:SetPoint("CENTER")
	mod:SetCamera(1)
...
function SetBackgroundModelWithClass(model, name, class)
    local nameupper = strupper(name);
	if class and class == 11 then
		--SetCharCustomizeBackground("world/azeroth/westfall/passivedoodads/outhouse/outhouse.m2");
		--SetCharSelectBackground("world/azeroth/westfall/passivedoodads/outhouse/outhouse.m2")
		-- old code
		local path = "Interface\\Glues\\Models\\UI_"..name.."\\UI_"..name..".m2";
		if ( model == CharacterCreate ) then
			SetCharCustomizeBackground(path);
		...
		-- end old
		local chrCreate = _G["CharacterCreate"]
		-- GlueParent
		-- try using correct 
		local m = newModelInt(model,1,nil,1,1,"world/azeroth/westfall/passivedoodads/outhouse/outhouse.m2",chrCreate)
		m:SetPosition(-0.7, 0.5, 0.2)
		m:SetFacing(0.707)
		m:SetModelScale(0.2)
